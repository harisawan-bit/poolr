...
from poolr.export.reports import export_to_word, export_to_latex
...
class PrismaPage(BasePage):
    ...
    def _build(self):
        ...
        ctk.CTkButton(btn_frame, text="📄 Export LaTeX", command=self._export_latex, height=32).pack(side="left", padx=4)
    ...
    def _build_flow_tab(self):
        ...
        ctk.CTkButton(btn_frame, text="🔄 Generate Diagram", command=self._draw_flow, height=36).pack(side="left", padx=8)
        ctk.CTkButton(btn_frame, text="💾 Save as SVG", command=self._save_flow_svg, height=36).pack(side="left", padx=8)
    ...
    def _build_grade_tab(self):
        ...
        ctk.CTkLabel(grade_frame, text="GRADE Evidence Profile", font=ctk.CTkFont(size=18, weight="bold")).pack(anchor="w", pady=(0, 12))
        
        btn_frame = ctk.CTkFrame(grade_frame, fg_color="transparent")
        btn_frame.pack(fill="x", pady=(0, 12))
        ctk.CTkButton(btn_frame, text="🔄 Auto-generate", command=self._generate_grade, height=36).pack(side="left", padx=4)
        ctk.CTkButton(btn_frame, text="📤 Export Table", command=self._export_grade, height=36).pack(side="left", padx=4)
    ...
    def _draw_flow(self):
        ...
        if vals["excluded_ta"] > 0:
            ...
            self.flow_canvas.create_rectangle(side_x, y, side_x + 250, y + box_h,
                                             fill=excluded_color, outline=excluded_border, width=2)
    ...
    def _generate_flow(self):
        ...
        assessed = sum(1 for r in ft if r.get("decision_reviewer1") is not None or r.get("decision_reviewer2") is not None)
    ...
    def _generate_grade(self):
        ...
        if not grade_data["assessments"]:
            ctk.CTkLabel(self.grade_table_frame, text="No meta-analysis results available for GRADE assessment").pack(pady=20)
            return
        
        # Table header
        headers = ["Outcome", "Studies", "Design", "Risk of Bias", "Inconsistency", "Indirectness", 
                   "Imprecision", "Pub. Bias", "Start", "Final", "Downgrades"]
        
        header_frame = ctk.CTkFrame(self.grade_table_frame)
        header_frame.pack(fill="x", padx=4, pady=4)
        for i, h in enumerate(headers):
            ctk.CTkLabel(header_frame, text=h, font=ctk.CTkFont(size=10, weight="bold")).grid(row=0, column=i, padx=4, pady=4)
        ...
        ctk.CTkLabel(self.grade_table_frame, text=f"Summary: {summary.get('total_outcomes', 0)} outcomes assessed | "
              ...
              font=ctk.CTkFont(size=11, weight="bold")).pack(pady=12)
