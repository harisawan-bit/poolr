...
class MetaPage(BasePage):
    ...
    def _build(self):
        ...
        ctk.CTkSegmentedButton(settings, values=["Random-effects", "Fixed-effect"], variable=self.model_var).pack(
        ...
        ctk.CTkSegmentedButton(settings, values=["OR", "RR", "RD", "MD", "SMD", "HR"], variable=self.measure_var).pack(
            side="left", padx=8, pady=12
        )
        ...
        ctk.CTkComboBox(
            settings, values=["DL", "REML", "PM", "HS", "SJ", "ML", "EB"], variable=self.method_var, width=120
        ...
        ctk.CTkComboBox(
            settings, values=["none", "design", "country", "year", "custom"], variable=self.subgroup_var, width=150
        ).pack(side="left", padx=8, pady=12)
        ...
        ctk.CTkComboBox(
            settings, values=["none", "egger", "begg", "trimfill"], variable=self.pub_bias_var, width=120
        ).pack(side="left", padx=8, pady=12)
        ...
        self.plot_label = ctk.CTkLabel(
            self.plot_frame, text="Forest/Funnel plot will appear here after analysis", font=ctk.CTkFont(size=13)
        )
        ...
        try:
            ...
            if not analysis_data:
                messagebox.showwarning(
                    "Warning", "No valid outcome data found. Ensure studies have outcome fields filled."
                )
                return
