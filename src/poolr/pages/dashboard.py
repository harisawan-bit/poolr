...
class DashboardPage(BasePage):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(header, text="Your systematic review & meta-analysis workspace", font=ctk.CTkFont(size=13)).pack(
            anchor="w", pady=(4, 0)
        )
        ...
        ctk.CTkLabel(actions_frame, text="Quick Actions", font=ctk.CTkFont(size=16, weight="bold")).pack(
        ...
        for i, (label, key) in enumerate(actions):
            btn = ctk.CTkButton(btn_frame, text=label, height=36, command=lambda k=key: self.app._select_page(k))
        ...
        ctk.CTkLabel(self.info_frame, text="Project Information", font=ctk.CTkFont(size=16, weight="bold")).pack(
            anchor="w", padx=16, pady=(12, 8)
        )
    ...
    def _update_info(self):
        ...
        if not self.app.project_path:
            ...
            lines = [
                ...
                f"Version: {meta.get('version', '0.3.0')}",
                "",
            ]
