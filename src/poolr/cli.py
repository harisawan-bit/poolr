...
def run_meta_analysis(project_data, measure="OR", model="random", method="DL"):
    ...
    print(f"  Pooled effect: {results['pooled']['effect']:.4f} "
...
def generate_prisma(project_data, output_dir):
    ...
    flow_data = {
        ...
        "sought": included_ta,
        "not_retrieved": 0,
        "assessed": included_ta,
        "excluded_ft": 0,
        "included": included_ta,
    }
...
def export_report(project_data, format_type, output_dir):
    ...
    if format_type == "word":
        export_to_word(project_data, output_dir / "report.docx")
        print(f"  ✓ Word report saved: {output_dir / 'report.docx'}")
    elif format_type == "latex":
        export_to_latex(project_data, output_dir / "report.tex")
...
Examples:
  ...
  poolr-cli ./my_project --export all --output ./dist/
    ...
    parser.add_argument("--method", default="DL", choices=["DL", "REML", "PM", "HS", "SJ", "ML", "EB"], help="Estimation method")
