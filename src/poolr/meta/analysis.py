...
class MetaAnalysis:
    ...
    def _pool_effects(
        self, studies: List[Dict], effects: List[float], variances: List[float], weights: List[float], k: int
    ) -> Dict:
        ...
        return {
            ...
            "heterogeneity": {"q": q, "df": df, "q_p": q_p, "i2": i2, "tau2": tau2, "tau": math.sqrt(tau2)},
            "subgroups": subgroups,
    ...
    def _test_publication_bias(self, effects, variances, k):
        ...
        if self.pub_bias in ["egger", "all"]:
            ...
            if len(set(precision)) > 1 and len(set(effects)) > 1:
                ...
                results["egger"] = {"intercept": intercept, "p_value": p, "significant": p < 0.05}
            else:
        ...
        if self.pub_bias in ["begg", "all"]:
            ...
            if len(set(effects)) > 1 and len(set(variances)) > 1:
                ...
                tau, p = 0.0, 1.0
            results["begg"] = {"tau": tau, "p_value": p, "significant": p < 0.05}

        # Trim and fill (simplified)
        if self.pub_bias in ["trimfill", "all"]:
            # Simplified - would need full implementation
            results["trimfill"] = {"note": "Trim-and-fill requires full implementation", "adjusted_effect": None}
