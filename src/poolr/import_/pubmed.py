...
class PubMedClient:
    ...
    def search(
        self, query: str, max_results: int = 100, sort: str = "relevance", date_range: Optional[tuple] = None
    ) -> List[str]:
        """
    ...
    def search_and_fetch(
        self, query: str, max_results: int = 100, date_range: Optional[tuple] = None
    ) -> List[PubMedRecord]:
...
class PubMedImporter:
    ...
    def import_to_project(
        ...
        screening_mode: str = "title_abstract",
    ) -> int:
        """
...
def show_pubmed_search_dialog(parent, app):
    ...
    ctk.CTkSegmentedButton(opts_frame, values=["Title/Abstract", "Full Text"], variable=mode_var).grid(
    ...
    def run_search():
        ...
        def worker():
            try:
                ...
                count = importer.import_to_project(app, query, max_results, date_range, mode_var.get())

                dialog.after(0, lambda: _on_complete(count))
            except Exception as e:
                dialog.after(0, lambda err=e: _on_error(err))
    ...
    ctk.CTkButton(btn_frame, text="Cancel", command=dialog.destroy, height=40, fg_color="gray").pack(
        side="left", padx=10
    )
