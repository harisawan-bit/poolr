...
def create_forest_plot(results: Dict[str, Any], figsize: tuple = (10, 12)) -> plt.Figure:
    ...
    # Draw pooled effect (diamond) in data coordinates
    # (axis itself is log-scaled for ratio measures, so no manual log transform)
    diamond_x = [pooled_lower, pooled_effect, pooled_upper, pooled_effect]
    diamond_y = [pooled_y, pooled_y + 0.2, pooled_y, pooled_y - 0.2]

    diamond = plt.Polygon(
        list(zip(diamond_x, diamond_y)), facecolor=pooled_color, edgecolor="darkgreen", linewidth=1.5, zorder=3
    )
    ...
    # Reference line (null effect): 1 for ratio measures on log axis, 0 otherwise
    null_line = 1.0 if use_log else 0.0
...
def create_prisma_flow_diagram(flow_data: Dict[str, int], figsize: tuple = (10, 14)) -> plt.Figure:
    ...
    def draw_box(x, y, w, h, text, fill_color=box_color, edge_color=border_color, fontsize=10):
        box = FancyBboxPatch(
            (x, y), w, h, boxstyle="round,pad=0.1", facecolor=fill_color, edgecolor=edge_color, linewidth=1.5
        )
        ax.add_patch(box)
    ...
    def draw_arrow(x1, y1, x2, y2):
        ax.annotate("", xy=(x2, y2), xytext=(x1, y1), arrowprops=dict(arrowstyle="->", color=arrow_color, lw=1.5))
    ...
    pos1 = draw_box(left_col, y, box_w, box_h, f"Records identified from\ndatabases (n = {identified})")
    y -= box_h + gap

    # Step 2: Before screening
    pos2 = draw_box(mid_col, y, box_w, box_h, f"Records before\nscreening (n = {before_screening})")
    ...
    if excluded_ta > 0:
        pos3_ex = draw_box(
            ...
            edge_color=excluded_border,
        )
        draw_arrow(pos2[0] + box_w / 2, pos2[1], pos3_ex[0] - box_w / 2, pos3_ex[1])

    # Step 4: Reports sought
    pos4 = draw_box(mid_col, y - box_h - gap, box_w, box_h, f"Reports sought for\nretrieval (n = {sought})")
    ...
    # Step 6: Assessed
    pos6 = draw_box(mid_col, y - box_h - gap, box_w, box_h, f"Reports assessed for\neligibility (n = {assessed})")
    ...
    # Title
    ax.text(
        5, 13.5, "PRISMA 2020 Flow Diagram", ha="center", va="center", fontsize=18, fontweight="bold", color="#1565C0"
    )
