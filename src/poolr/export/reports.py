...
try:
    from pylatex import Command, NoEscape, Package, Section, Subsection
...
def export_to_word(project_data: Dict[str, Any], project_path: Path, output_path: Optional[Path] = None) -> Path:
    ...
    p = doc.add_paragraph()
    p.add_run("Project Version: ").bold = True
...
def _add_abstract(doc, project_data):
    ...
    abstract_text += "We conducted a systematic review and meta-analysis to evaluate "
    ...
    if meta_results:
        ...
        p.add_run(
            f"We searched {', '.join(project_data.get('protocol', {}).get('databases', 'databases').split(', '))} "
        ...
        p.add_run(
            f"{len(project_data.get('screening', {}).get('title_abstract', []))} records were screened, "
...
def _add_methods(doc, project_data):
    ...
    if rob_tools:
        ...
        doc.add_paragraph(
            "Risk of bias was assessed using RoB 2 for RCTs, NOS for cohort studies, and PROBAST for diagnostic/prognostic studies."
        )
    ...
    if meta_results.get("subgroups"):
        doc.add_paragraph(
            f"Subgroup analyses were performed by {project_data.get('meta', {}).get('subgroup', 'study design')}."
        )
...
def _add_results(doc, project_data):
    ...
    if meta_results.get("subgroups"):
        ...
        for sg in meta_results["subgroups"]:
            ...
            p.add_run(
                f"{sg['measure']} = {sg['effect']:.2f} (95% CI: {sg['ci_lower']:.2f}–{sg['ci_upper']:.2f}), k={sg['k']}"
            )
    ...
    if meta_results.get("publication_bias"):
        ...
        for test, val in meta_results["publication_bias"].items():
            if isinstance(val, dict):
                ...
                p.add_run(f"p = {p_value:.4f}" if isinstance(p_value, (int, float)) else "p = N/A")
...
def _add_meta_results(doc, results):
    ...
    if pooled:
        ...
        if hetero:
            ...
            p.add_run(
                f"Q = {hetero.get('q', 0):.2f} (df = {hetero.get('df', 0)}), P = {hetero.get('q_p', 1):.4f}; "
...
def _add_grade_table(doc, project_data):
    ...
    # Publication bias (may be stored as None when tests were skipped)
    pub_bias = meta_results.get("publication_bias") or {}
    ...
    data = [
        ...
        (
            "Risk of bias",
            "Serious" if any("Risk of bias" in d for d in downgrades) else "Not serious",
            "↓" if any("Risk of bias" in d for d in downgrades) else "",
            next((d for d in downgrades if "Risk of bias" in d), "No serious concerns"),
        ),
        (
            "Inconsistency",
            "Serious" if any("Inconsistency" in d for d in downgrades) else "Not serious",
        ...
        (
            "Imprecision",
            "Serious" if any("Imprecision" in d for d in downgrades) else "Not serious",
            "↓" if any("Imprecision" in d for d in downgrades) else "",
            next((d for d in downgrades if "Imprecision" in d), "No serious concerns"),
        ),
        (
            "Publication bias",
            "Serious" if any("Publication bias" in d for d in downgrades) else "Not serious",
...
def export_to_latex(project_data: Dict[str, Any], project_path: Path, output_path: Optional[Path] = None) -> Path:
    ...
    with doc.create(Section("Introduction")):
        ...
        with doc.create(Subsection("Objectives")):
            pico = project_data.get("pico", {})
            doc.append(
                f"To evaluate {pico.get('intervention', '[intervention]')} vs {pico.get('comparator', '[comparator]')} "
    ...
    if args.run_meta:
        ...
        if studies:
            ...
            if has_binary:
                ...
                print(
                    f"Pooled OR = {results['pooled']['effect']:.3f} "
