/* eslint-disable */
// poolr UI kit — single module. Exports the legacy shell primitives
// (Card w/ title+right, Select, Textarea, Pill, EmptyState) AND the
// shadcn-flavored family the ported kokonutui components expect
// (Button, Input, CardContent, Drawer, DropdownMenu). Token-driven,
...
export function EmptyState({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-[5px] border border-dashed border-[var(--color-border)] px-4 py-8 text-center text-[12.5px] text-[var(--color-text-muted)]">
      {children}
    </div>
  );
}
...
const DrawerDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p ref={ref} className={cn("text-[12.5px] text-[var(--color-text-muted)] leading-relaxed", className)} {...props} />
  )
);
